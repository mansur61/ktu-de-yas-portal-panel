...
if (Test-PortInUse 5000) {
    ...
    $cmd = "`$env:DOTNET_ENVIRONMENT='Development'; dotnet run --project `"$($backendRoot.Path)\src\timeseries-service\TimeseriesService.csproj`" -c Release --no-build"
...
if (Test-PortInUse 5056) {
    ...
    $cmd = "`$env:DOTNET_ENVIRONMENT='Development'; dotnet run --project `"$panelCsproj`" -c Release --no-build"
